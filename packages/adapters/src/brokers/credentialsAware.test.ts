...
function memoryCredentials(
  stored: Partial<Record<"ibkr" | "trading212", unknown>>,
): CredentialStore {
  return {
...
test("stored credentials reach the broker adapters together with env config", async () => {
  const baseUrl = await serve((request, response) => {
    ...
    if ((request.url ?? "").startsWith("/api/v0/equity/account"))
  ...
  try {
    ...
    expect(log).toHaveBeenCalledWith(
      expect.stringContaining('"event":"investing.trading212.http"'),
    );
  } finally {
    log.mockRestore();
  }
...
test("INVESTING_SYNC_BUDGET_MS overrides the Vercel default", () => {
  expect(
    trading212DeadlineMs(
      (name) =>
        name === "INVESTING_SYNC_BUDGET_MS" ? "240000" : name === "VERCEL" ? "1" : undefined,
      1_000,
    ),
  ).toBe(241_000);
});
