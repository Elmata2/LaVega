...
export function createIndexedDbPriceStore(dbName = DEFAULT_DB_NAME): PriceStore {
  return {
    async getRange(tenantId, symbol, from, to) {
      ...
      const rows: StoredPriceBar[] = await index.getAll(IDBKeyRange.bound([tenantId, symbol, from ?? ""], [tenantId, symbol, to ?? "￿"]));
      db.close();
    ...
    async lastDate(tenantId, symbol) {
      const db = await openPriceDb(dbName);
      const index = db.transaction(STORE_NAME).store.index("tenant-symbol-date");
      const rows: StoredPriceBar[] = await index.getAll(IDBKeyRange.bound([tenantId, symbol, ""], [tenantId, symbol, "￿"]));
      db.close();
    ...
    async upsert(tenantId, bars) {
      ...
      await Promise.all(bars.map((bar) => tx.store.put({ ...bar, tenantId } satisfies StoredPriceBar)));
      await tx.done;
      db.close();
    },
    async purgeAll() {
