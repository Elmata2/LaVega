...
test("a bank's own document beats the live comparison figure for the same product", async () => {
  const live = { asOf: "2026-08-19", rates: [{ bank: "ABN AMRO", product: "Direct Sparen", ratePct: 1.3, freeWithdrawal: true }] };
  const p = createRatesProvider({
    url: "https://x/api/rates",
    fetchFn: (async () => ({ ok: true, json: async () => live })) as unknown as typeof fetch,
    cache: { get: () => null, set: () => {} },
    catalogueRates: [{ bank: "ABN AMRO", product: "Direct Sparen", ratePct: 1.25, freeWithdrawal: true, sourceUrl: "https://abn/fid.pdf", asOf: "2025-05-01" }],
  });
...
test("a product only the scrape knows is kept, not dropped by the catalogue", async () => {
  const live = { asOf: "2026-08-19", rates: [{ bank: "Klarna", product: "Flex rekening", ratePct: 1.95, freeWithdrawal: true }] };
  const p = createRatesProvider({
    url: "https://x/api/rates",
    fetchFn: (async () => ({ ok: true, json: async () => live })) as unknown as typeof fetch,
    cache: { get: () => null, set: () => {} },
    catalogueRates: [{ bank: "Triodos", product: "Internet Sparen", ratePct: 1.15, freeWithdrawal: true, sourceUrl: "https://t/x", asOf: "2026-02-01" }],
  });
  const got = await p.getRates();
