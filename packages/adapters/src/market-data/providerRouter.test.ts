...
import { firstProviderResult, hasProblems, type Provider } from "./providerRouter.js";
...
test("lane returns first non-null result in priority order with provenance", async () => {
  ...
  const lane = [
    ...
    {
      ...provider("marketstack", 20, { close: 101 }),
      get: async () => {
        called.push("marketstack");
        return { close: 101 };
      },
    },
  ];

  await expect(firstProviderResult(lane, { symbol: "ASML" })).resolves.toEqual({
    sourceKey: "marketstack",
    value: { close: 101 },
  });
...
test("lane skips provider errors and continues", async () => {
  ...
  const lane = [
    ...
    provider("fallback", 10, "ok"),
  ];

  await expect(firstProviderResult(lane, {}, log)).resolves.toEqual({
    sourceKey: "fallback",
    value: "ok",
  });
...
test("price lane falls through provider problems to a healthy fallback", async () => {
  ...
  const first: Provider<unknown, PriceResult> = {
    sourceKey: "yahoo",
    priority: 20,
    get: async () => ({ bars: [], problems: ["Yahoo blocked"] }),
  };
  const fallback: Provider<unknown, PriceResult> = {
    sourceKey: "fallback",
    priority: 10,
    get: async () => ({ bars: [{ close: 100 }], problems: [] }),
  };
...
test("price lane preserves provider problems when all providers fail", async () => {
  const lane = [
    provider<unknown, { bars: unknown[]; problems: string[] }>("yahoo", 20, {
      bars: [],
      problems: ["Yahoo blocked"],
    }),
    provider<unknown, { bars: unknown[]; problems: string[] }>("fallback", 10, {
      bars: [],
      problems: ["Fallback unavailable"],
    }),
  ];
