...
export type FrankfurterFxProvider = Provider<FxRequest, FxProviderResult> & { getLatestRate(): Promise<FxRate> };
export function createFrankfurterFxProvider(input: { client?: FxHttpClient; now?: () => number } = {}): FrankfurterFxProvider {
  ...
  const getLatestRate = async (): Promise<FxRate> => { try { return await loadRate(); } catch { return cached?.rate ?? FX_RATE_FALLBACK; } };
