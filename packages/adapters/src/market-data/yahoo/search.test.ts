...
test("Yahoo benchmark search reuses crumb client and confirms missing currency", async () => {
  const fetchJsonWithCrumb = vi.fn()
    .mockResolvedValueOnce({ quotes: [{ symbol: "^AEX", shortname: "AEX", exchDisp: "Amsterdam", quoteType: "INDEX" }] })
...
test("Yahoo benchmark search falls back to curated European list", async () => {
  const result = await searchYahooBenchmarks("AEX", { client: { fetchJsonWithCrumb: vi.fn().mockRejectedValue(new Error("blocked")) } as never });
