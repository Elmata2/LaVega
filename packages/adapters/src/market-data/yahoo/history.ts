...
export async function loadYahooPriceHistory(input: { ticker: string; exchange: string; range?: YahooRange; interval?: YahooInterval; client?: YahooHttpClient }): Promise<YahooPricePoint[]> {
  const client = input.client ?? new YahooHttpClient();
  let lastError: unknown;
  for (const symbol of getYahooSymbolsToTry(input.ticker, input.exchange)) {
    try {
      const url = `${CHART_URL}${encodeURIComponent(symbol)}?range=${input.range ?? "5y"}&interval=${input.interval ?? "1d"}&events=div%2Csplits`;
