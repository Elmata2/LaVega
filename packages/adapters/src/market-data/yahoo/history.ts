...
export type YahooPriceHistory = { symbol: string; currency: string | null; points: YahooPricePoint[] };
export type YahooPriceHistoryInput = { ticker: string; exchange: string; isin?: string; range?: YahooRange; interval?: YahooInterval; from?: string; to?: string; client?: YahooHttpClient };
const CHART_URL = "https://query1.finance.yahoo.com/v8/finance/chart/";

export async function loadYahooPriceHistory(input: YahooPriceHistoryInput): Promise<YahooPriceHistory> {
  ...
  for (const symbol of await yahooSymbolCandidates(input, client)) {
    try {
      ...
      const history = { symbol, currency: result.meta?.currency ?? null, points: mapYahooChart(result) };
      ...
      if (history.points.length === 0) { empty ??= history; continue; }
      return history;
...
async function yahooSymbolCandidates(input: YahooPriceHistoryInput, client: YahooHttpClient): Promise<string[]> {
