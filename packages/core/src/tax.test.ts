...
test("computeVatSetAside: 21% net-VAT on Q2 margin, due 07-31", () => {
  ...
  expect(f).toMatchObject({ source: "vat", sign: -1, status: "expected", dueDate: "2026-07-31", entity: "BV1" });
...
const inv = (o: Partial<Invoice> = {}): Invoice => makeInvoice({
  entity: "BV1", direction: "in", counterparty: "Klant", issueDate: "2026-04-10",
  dueDate: "2026-05-10", amount: 1210, vatAmount: 210, currency: "EUR",
...
test("gemengde tarieven: unknown stays null and says why — never a zero", () => {
  const p = vatPosition({ txs: [tx("2026-05-01", 99999)], settings: settings({ mixedRates: true }), asOf: "2026-06-20" });
...
test("the bases are NEVER summed: one basis wins whole", () => {
  ...
  const figures = sumTaxFigures([sheetRow("2026-04-01", { vatChargedCents: 310000, vatPaidCents: 42000 })], "2026-04-01", "2026-06-30");
  ...
  const manual = vatPosition({ txs, settings: settings({ vatBasis: "factuurstelsel", manualCents: 500 }), asOf: "2026-06-20", figures, invoices });
  ...
  expect(computeVatSetAside(txs, settings({ manualCents: 500 }), "2026-06-20", figures, invoices)!.status).toBe("confirmed");
});
...
test("every figure this feature emits carries its provenance", () => {
  ...
  expect(vatPosition({ txs: [], settings: settings({ country: "DE" }), asOf: "2026-06-20" }).vatLabel).toBe("USt");
});

test("figures for the wrong window are refused AND the reason reaches the screen", () => {
  const figures = sumTaxFigures([sheetRow("2026-01-15", { vatChargedCents: 210000, vatPaidCents: 0 })], "2026-01-01", "2026-03-31");
  const p = vatPosition({ txs: [tx("2026-04-10", 12100)], settings: settings(), asOf: "2026-06-20", figures });
  expect(p.basis).toBe("proxy");
...
test("computeTaxReservations carries the invoices through to the BTW figure", () => {
  ...
  const flows = computeTaxReservations({
    txs: [], settings: settings({ vatBasis: "factuurstelsel" }), asOf: "2026-06-20", invoices,
  });
...
test("no bank movements in the window is UNKNOWN, not a measured zero", () => {
  ...
  const p = vatPosition({ txs: [tx("2026-01-05", 12100)], settings: settings(), asOf: "2026-06-20" });
  expect(p.netCents).toBeNull();
  expect(p.direction).toBe("onbekend");
  expect(p.note).toBe("geen-banktransacties");
  // a window that DOES have movements which happen to net to zero is a real zero
  const flat = vatPosition({ txs: [tx("2026-04-10", 1000), tx("2026-04-11", -1000)], settings: settings(), asOf: "2026-06-20" });
