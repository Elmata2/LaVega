...
test("position return uses trade-date FX, average cost, sell fees, and dividends", () => {
  ...
  expect(
    calculatePositionReturn(6, 120, trades, dividends, "EUR", rates, {
      valuationDate: "2027-01-02",
    }),
  ).toEqual({
    status: "available",
...
test("position return stays unavailable for incomplete history and a zero denominator", () => {
  expect(calculatePositionReturn(2, 20, [trade({ quantity: 1 })], [], "EUR", rates).status).toBe(
    "missing-cost",
  );
  expect(
    calculatePositionReturn(10, 20, [trade({ commission: null })], [], "EUR", rates).status,
  ).toBe("missing-cost");
  const zero = calculatePositionReturn(
    0,
    0,
    [
      trade({ amount: 0, price: 0, commission: 0 }),
      trade({ side: "sell", amount: 0, price: 0, commission: 0 }),
    ],
    [],
    "EUR",
    rates,
...
test("unpriced open position retains known realized and dividend components", () => {
  const trades = [
    ...
    trade({ id: "sell", side: "sell", quantity: 1, amount: 15, currency: "EUR", commission: 1 }),
  ];
  const dividends: Dividend[] = [
    {
      id: "dividend",
  ...
  expect(calculatePositionReturn(1, null, trades, dividends, "EUR", rates)).toMatchObject({
    status: "unpriced",
    remainingCostBasis: 10,
    realizedGain: 4,
    dividendsReceived: 2,
    unrealizedGain: null,
    totalReturn: null,
...
test("current positions omit closed holdings and expose price quality, EUR weight, and missing FX", () => {
  ...
  expect(result[0]).toMatchObject({
    marketValue: 120,
    portfolioWeight: 120 / 170,
    priceStatus: "forward-filled",
  });
  expect(result[1]).toMatchObject({
    marketValue: 50,
    portfolioWeight: 50 / 170,
    priceStatus: "priced",
  });
  const expectedSinceFirstBuy = calculatePositionReturn(
    6,
    120,
    [trades[0]!],
    dividends,
  ...
  const noFxProvider = buildCurrentPositions({
    positions: [positions[0]!],
    trades: [trades[0]!],
    dividends: [],
    priceBars: bars,
  ...
  expect(noFxProvider[0]).toMatchObject({
    marketValue: null,
    portfolioWeight: null,
    priceStatus: "missing-fx",
    returns: { status: "missing-fx" },
  ...
  const eurWithoutFx = buildCurrentPositions({
...
test("price becomes unknown after five business days", () => {
  ...
  const result = buildCurrentPositions({
    positions: [position],
    trades: [trade({ quantity: 1, amount: 10, currency: "EUR", commission: 0 })],
    dividends: [],
    priceBars: [{ symbol: "ACME", date: "2026-01-02", close: 10, currency: "EUR" }],
    presentationCurrency: "EUR",
    fxRates: rates,
    today: "2026-01-13",
  });
