...
test("amountCents stays an integer for an even occurrence count (median of two middles is rounded)", () => {
  const txs: Tx[] = [
    ...
    tx("2", "2026-04-25", -100.0, "Saas"),
    tx("3", "2026-05-25", -100.01, "Saas"),
    tx("4", "2026-06-25", -100.02, "Saas"),
  ];
...
test("monthly cadence steps by CALENDAR month, not 30 days — no drift over a quarter", () => {
  ...
  expect(streamOccurrences(s, "2026-06-01", "2026-08-31")).toEqual([
    "2026-06-25",
    "2026-07-25",
    "2026-08-25",
  ]);
...
test("a month-end stream clamps to short months and RETURNS to the 31st", () => {
  ...
  expect(streamOccurrences(s, "2026-03-01", "2026-05-31")).toEqual([
    "2026-03-28",
    "2026-04-28",
    "2026-05-28",
  ]);
  const jan31 = { ...s, lastDate: "2026-01-31" };
  expect(streamOccurrences(jan31, "2026-02-01", "2026-05-31")).toEqual([
    "2026-02-28",
    "2026-03-31",
    "2026-04-30",
    "2026-05-31",
...
test("a recently missed payment is rolled forward, not silently dropped", () => {
  ...
  expect(streamOccurrences(s, "2026-07-10", "2026-10-09")).toEqual([
    "2026-07-11",
    "2026-08-01",
    "2026-09-01",
    "2026-10-01",
  ...
  expect(streamOccurrences(s, "2026-07-02", "2026-10-01")).toEqual([
    "2026-08-01",
    "2026-09-01",
    "2026-10-01",
  ]);
...
test("quiet weeks count as real zeros: two identical burst weeks are not a steady scope", () => {
  ...
  const txs: Tx[] = [
    tx("1", "2026-01-01", -1000, "Verhuurder"),
    tx("2", "2026-02-01", -1000, "Verhuurder"),
    tx("3", "2026-03-01", -1000, "Verhuurder"),
    tx("4", "2026-04-01", -1000, "Verhuurder"),
...
test("an overdue, still-unpaid scheduled flow is reported — not silently dropped from the projection", () => {
  ...
  const overdue = makeScheduledFlow({
    entity: "BV1",
    label: "BTW Q2",
    sign: -1,
    amountCents: 30000,
...
test("atRisk: the band reaches below the buffer while the expected line still clears it", () => {
  ...
  const accounts: Account[] = [
    { key: "A1", iban: "A1", name: "x", bank: "", entity: "BV1", currency: "EUR", balance: 6200 },
  ];
