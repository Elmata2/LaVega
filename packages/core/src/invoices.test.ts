...
test("reconcileInvoices: the invoice number in the description settles it even when the name does not overlap", () => {
  ...
  const out = reconcileInvoices(invoices, [
    txd("t1", "2026-08-28", -1210, "KPN B.V.", "Factuurnr 2026-0042 termijn augustus"),
  ]);
  expect(out[0]).toMatchObject({ status: "paid", matchedTxId: "t1" });
});
...
test("reconcileInvoices: a short invoice number is not an identifier and cannot stand in for the name", () => {
  ...
  const out = reconcileInvoices(invoices, [
    txd("t1", "2026-08-28", -1210, "KPN B.V.", "termijn 7 augustus"),
  ]);
...
test("reconcileInvoices: the invoice number does not override amount, sign or the date window", () => {
  ...
  expect(
    reconcileInvoices(invoices, [txd("t1", "2026-08-28", -900, "KPN", number)])[0].status,
  ).toBe("expected"); // wrong amount
  expect(
    reconcileInvoices(invoices, [txd("t2", "2026-08-28", 1210, "KPN", number)])[0].status,
  ).toBe("expected"); // wrong sign
  expect(
    reconcileInvoices(invoices, [txd("t3", "2026-05-01", -1210, "KPN", number)])[0].status,
  ).toBe("expected"); // outside the window
});
