...
import {
  answerCrossScope, businessCostsPaidPrivately, crossScopeStreamKey, crossScopeTransfers,
  type CrossScopeAnswer, type CrossScopeReport,
} from "./crossScope.js";
...
const acc = (key: string, entity: string, iban: string): Account =>
  ({ key, iban, name: key, bank: "ING", entity, currency: "EUR", balance: null });
...
const tx = (
  id: string, accountKey: string, date: string, amount: number,
  counterparty = "", description = "", currency = "EUR",
): Tx => ({ id, accountKey, date, amount, currency, counterparty, description, category: "", manual: false });
...
test("een gematchte overboeking over de grens telt ÉÉN keer, met twee benen eronder", () => {
  ...
  const overLegs = r.crossings.flatMap((x) => x.legs).reduce((n, l) => n + Math.abs(l.signedCents), 0);
...
test("eenbenig: de BV betaalt rechtstreeks een privé-creditcard — gemeld als ongematcht, met de rekening als bewijs", () => {
  const r = measured(crossScopeTransfers({
    accounts: [BV, PRIVE, AMEX],   // de Amex is bekend; zijn afschrift is nooit geïmporteerd
    txs: [tx("o1", BV.key, "2026-05-02", -1200.5, "American Express", `Aflossing kaart ${AMEX_IBAN}`)],
    profiles: PROFILES, asOf: ASOF,
  }));
  expect(r.crossings).toHaveLength(1);
  const c = r.crossings[0];
...
test("een bewijsloze afboeking naar een rekening die NIET in je vault staat wordt geteld, niet stil weggelaten", () => {
  ...
  const bekend = measured(run([tx("o1", BV.key, "2026-02-05", -12400, "", `SEPA Overboeking ${PRIVE_IBAN}`)]));
  ...
  const eigen = measured(run([tx("o1", BV.key, "2026-02-05", -12.5, "ING", `Kosten rekening ${BV_IBAN}`)]));
...
test("niets als zakelijk gemarkeerd: een eigen staat, en de meting is niet eens bereikbaar", () => {
  ...
  expect(businessCostsPaidPrivately({ accounts: [BV, PRIVE], txs: [], profiles: [], asOf: ASOF }).state)
...
test("alles zakelijk: de andere kant van de grens ontbreekt, en dat is óók een eigen staat", () => {
  const r = crossScopeTransfers({
    ...
    profiles: [{ entity: "BV1", scope: "business" }, { entity: "BV2", scope: "business" }],
    asOf: ASOF,
  });
...
test("een rekening zonder entiteit, en een transactie zonder rekening: geteld, nooit stilzwijgend privé genoemd", () => {
  ...
  const leeg = crossScopeTransfers({
    accounts: [GEEN], txs: [tx("x1", GEEN.key, "2026-06-01", -400, "Coolblue", "")],
    profiles: [], asOf: ASOF,
  });
...
test("zijn antwoord op een stroom wint van elke latere gok, en opnieuw draaien vraagt het niet nog eens", () => {
  ...
  answers = answerCrossScope(answers, [{ target: stroom, kind: "salaris", source: "agent", updatedAt: "2026-08-24" }]);
  ...
  expect(measured(run(txs, { answers: eenmalig })).crossings.map((c) => c.kind)).toEqual(["dividend", "salaris"]);
});
...
test("zijn eigen naam op de regel telt alleen mét een naam die hij zelf gaf, en verzint zonder naam niets", () => {
  ...
  const familie = measured(run([tx("o2", BV.key, "2026-07-02", -3000, "B Steunenberg", "")], { names }));
...
test("twee ECHTE overboekingen van gelijke omvang uit VERSCHILLENDE ondernemingen worden niet tot één spiegel geknepen", () => {
  ...
  const p = measured(crossScopeTransfers({
...
test("bedragen blijven hele centen — nergens een float", () => {
  ...
  const alle = [
    r.totalCents, r.matchedCents, r.unmatchedCents,
...
test("zakelijke kosten van een privérekening: dezelfde tegenpartij aan beide kanten van de grens", () => {
  ...
  expect(Object.keys(row).sort()).toEqual([
    "businessCents", "businessCount", "firstDate", "label", "lastDate",
    "merchant", "personalCents", "personalCount", "personalTxIds",
  ]);
});
