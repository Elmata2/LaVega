...
test("vergelijkt een lopende maand met dezelfde eerste dagen van elke eerdere maand", () => {
  const history = MONTHS.flatMap((m) => [
    row(`${m}-05`, "Uit eten", 50),
    row(`${m}-25`, "Uit eten", 300),
  ]);
  ...
  expect(r.historyCents).toEqual(Array.from({ length: 10 }, () => 5_000)); // de € 300 van de 25e telt niet mee
...
test("een uitgave van nul is een gemeten nul, geen weigering", () => {
  ...
  const out = categorySpendPercentiles(
    [...boodschappen, ...vakantie, row("2026-08-31", "Boodschappen", 175)],
    {
      asOf: "2026-08-31",
      coverage: { start: "2025-10-01", end: "2026-08-31" },
    },
