...
const rente = (date: string, amount: number): Tx => ({
  ...
  currency: "EUR",
  counterparty: "Rente",
  description: "Rente spaarrekening",
  category: "",
  manual: false,
});
...
test("detectInterestRate: implied % from trailing-year rente credits vs balance", () => {
  ...
  expect(
    detectInterestRate(acc({ balance: 0 }), [rente("2026-06-01", 150)], "2026-08-01"),
  ).toBeNull();
});

test("resolveAccountRate: manual > detected > assumed(0 for betaal) > unknown", () => {
  expect(resolveAccountRate(acc({ interestRate: 0.8 }), [], "2026-08-01")).toEqual({
    ratePct: 0.8,
    source: "manual",
  });
  expect(resolveAccountRate(acc({ type: "Betaalrekening" }), [], "2026-08-01")).toEqual({
    ratePct: 0,
    source: "assumed",
  });
  expect(resolveAccountRate(acc({ type: "Spaarrekening" }), [], "2026-08-01")).toEqual({
    ratePct: null,
    source: "unknown",
  });
  expect(
    resolveAccountRate(acc({ type: "Spaarrekening" }), [rente("2026-06-01", 150)], "2026-08-01"),
  ).toEqual({ ratePct: 1.5, source: "detected" });
});
...
test("resolveAccountRate: savings at a known bank estimates the current rate from that bank's standard tariff", () => {
  ...
  expect(resolveAccountRate(ing, [], "2026-08-01", NL_SAVINGS_RATES)).toEqual({
    ratePct: 1.25,
    source: "benchmark",
  });
  ...
  expect(
    resolveAccountRate(
      acc({ type: "Spaarrekening", bank: "Onbekende Bank" }),
      [],
      "2026-08-01",
      NL_SAVINGS_RATES,
    ).source,
...
test("detectInterestRate: implausible rate (tiny balance vs normal interest) is discarded -> benchmark", () => {
  ...
  expect(
    resolveAccountRate(tiny, [rente("2026-06-01", 16.5)], "2026-08-01", NL_SAVINGS_RATES),
  ).toEqual({ ratePct: 1.25, source: "benchmark" });
});
...
describe("mergeRateSources", () => {
  ...
  const cmp: RateBenchmark = {
    bank: "ABN AMRO",
    product: "Direct Sparen",
    ratePct: 1.3,
    freeWithdrawal: true,
  };
  const bun: RateBenchmark = {
    bank: "ABN AMRO",
    product: "Direct Sparen",
    ratePct: 1.25,
    freeWithdrawal: true,
  ...
  test("the bank's own document beats the comparison site", () => {
    const out = mergeRateSources(
      { rates: [cat], provenance: "catalogue" },
      { rates: [cmp], provenance: "comparison" },
    );
  ...
  test("a product only one source knows is KEPT, not dropped", () => {
    ...
    const only: RateBenchmark = {
      bank: "Klarna",
      product: "Flex rekening",
      ratePct: 1.95,
      freeWithdrawal: true,
    };
    const out = mergeRateSources(
      { rates: [cat], provenance: "catalogue" },
      { rates: [cmp, only], provenance: "comparison" },
    );
  ...
  test("two products of the SAME bank are not merged into one", () => {
    ...
    const fixed: RateBenchmark = {
      bank: "ABN AMRO",
      product: "Depositosparen",
      ratePct: 2.4,
  ...
  test("differently-punctuated names of the same product still merge", () => {
    ...
    const out = mergeRateSources(
      { rates: [cat], provenance: "catalogue" },
      { rates: [messy], provenance: "comparison" },
    );
    expect(out).toHaveLength(1);
...
describe("matching an account to its own bank's rate", () => {
  const cat: RateBenchmark[] = [
    {
      bank: "ING Bank",
      product: "Oranje Spaarrekening",
      ratePct: 1.25,
      freeWithdrawal: true,
      sourceUrl: "https://ing/x.pdf",
      asOf: "2026-01-01",
    },
    {
      bank: "ABN AMRO Bank",
      ...
      ratePct: 1.25,
      freeWithdrawal: false,
      sourceUrl: "https://abn/x.pdf",
      asOf: "2025-05-01",
    },
    {
      bank: "Open Bank",
      product: "Open Spaarrekening",
      ratePct: 1.8,
      freeWithdrawal: true,
      sourceUrl: "https://ob/x",
      asOf: "2026-08-01",
    },
  ];
  ...
  test("bank 'ING' finds the catalogue's 'ING Bank' — the miss behind the 0%", () => {
    ...
    const r = resolveAccountRate(
      savings({ bank: "ING", name: "NL95INGB0674843703" }),
      [],
      "2026-08-20",
      cat,
    );
  ...
  test("a WRONG bank match is worse than the 0% — 'Trading 212' never gets ING's rate", () => {
    ...
    expect(resolveAccountRate(savings({ bank: "Trading 212" }), [], "2026-08-20", cat).source).toBe(
      "unknown",
    );
    expect(resolveAccountRate(savings({ bank: "Bigbank" }), [], "2026-08-20", cat).source).toBe(
      "unknown",
    );
  });

  test("matches both directions and ignores punctuation, spacing and accents", () => {
    expect(resolveAccountRate(savings({ bank: "ABN AMRO" }), [], "2026-08-20", cat).ratePct).toBe(
      1.25,
    );
    expect(
      resolveAccountRate(savings({ bank: "ABN-Amro Bank N.V." }), [], "2026-08-20", cat).ratePct,
    ).toBe(1.25);
    // "Openbank" and "Open Bank" are the same bank written two ways.
    expect(resolveAccountRate(savings({ bank: "Openbank" }), [], "2026-08-20", cat).ratePct).toBe(
      1.8,
    );
  });
  ...
  test("the account's own name picks the product when a bank has several", () => {
    ...
    expect(
      resolveAccountRate(
        savings({ bank: "NIBC", name: "NIBC Spaarrekening" }),
        [],
        "2026-08-20",
        two,
      ).ratePct,
  ...
  test("an account we only GUESSED is a betaalrekening still names what its bank pays on savings", () => {
    ...
    const r = resolveAccountRate(
      acc({ bank: "ING", name: "NL88INGB0793113504", type: "Betaalrekening" }),
      [],
      "2026-08-20",
      cat,
    );
  ...
  test("a teaser with no known standard rate is not used as an existing customer's rate", () => {
    ...
    expect(
      resolveAccountRate(savings({ bank: "Trade Republic" }), [], "2026-08-20", promoOnly).source,
    ).toBe("unknown");
...
describe("promos are shown as well as ranked past", () => {
  ...
  test("no promo anywhere means no promo figure — not a zero-euro promise", () => {
    const r = analyzeInterest(
      [acc({ type: "Betaalrekening", balance: 50000 })],
      [],
      [pool[1]],
      "2026-08-20",
    );
...
describe("capitalAtRisk", () => {
  const fund: RateBenchmark = {
    bank: "N26",
    product: "flexible cash fund",
    ratePct: 2.32,
    freeWithdrawal: true,
    capitalAtRisk: true,
  };
  const deposit: RateBenchmark = {
...
const savings = (over: {
  ...
  fields: over.fee
    ? {
        accountFee: {
          value: over.fee.value,
          period: over.fee.period,
          route: "provider-pdf",
...
describe("de rekening waar het advies heen wijst kost zelf ook geld", () => {
  ...
  test("MAAND TEGEN JAAR: dezelfde prijs, twaalf keer zo groot als de eenheid wegvalt", () => {
    ...
    // genegeerd.
    const perMaand = analyzeInterest(
      IDLE,
      [],
      [TARGET],
      "2026-08-01",
    ...
    expect(
      perJaar.net!.kind !== "gross-cost-unknown" &&
        perJaar.net!.basis.kind === "recurring" &&
        perJaar.net!.basis.period,
    ).toBe("jaar");
  });
  ...
  test("KOSTEN ONBEKEND: het brutobedrag blijft staan en het woord netto valt NIET", () => {
    ...
    const fees = productFeesById([
      savings({ id: "test-spaar", product: "Testbank Spaarrekening" }),
    ]);
    const r = analyzeInterest(IDLE, [], [TARGET], "2026-08-01", undefined, fees);
  ...
  test("zonder winst is er niets te verrekenen, en dan is er geen net", () => {
    ...
    // niemand overwoog.
    const r = analyzeInterest(
      [acc({ key: "B", type: "Betaalrekening", balance: 0 })],
      [],
      [TARGET],
      "2026-08-01",
    );
  ...
  test("de prijs mag op de PAKKETRIJ staan — dezelfde matcher als het reisblok", () => {
    ...
    const fees = productFeesById([
      savings({
        id: "testbank-plus-betaalpas",
        product: "Testbank Plus betaalpas",
        issuer: "Testbank N.V.; Mastercard Debit",
      }),
      savings({
        id: "testbank-plus",
        product: "Testbank Plus",
        fee: { value: 4.5, period: "maand" },
      }),
    ]);
