...
test("a prepayment deadline gets the same ladder as a BTW deadline, in its own words", () => {
  const prepay = makeScheduledFlow({ entity: "BV1", label: "Vorauszahlung 3/4 2026", sign: -1, amountCents: 2_500_000, dueDate: "2026-08-10", source: "prepayment", status: "expected" });
  const alerts = computeAlerts({ accounts: [acc("A", 1000)], asOf: "2026-08-01", bufferCents: 0, forecast: fc({}), scheduledFlows: [prepay] });
...
test("a stale hand-kept balance becomes an alert whose detail IS the question (item 7, low-trust)", () => {
  const amex = makeRewardsBalance({ program: "American Express Membership Rewards", points: 240000, updatedAt: "2026-01-10" });
  const alerts = computeAlerts({ accounts: [acc("A", 100)], asOf: "2026-08-01", bufferCents: 0, forecast: fc({}),
...
test("a merely-due balance is info, a fresh one is nothing, and a money problem still outranks both", () => {
  ...
  const fresh = makeRewardsBalance({ program: "Flying Blue", points: 1000, updatedAt: "2026-07-20" });
  const alerts = computeAlerts({ accounts: [acc("A", 100)], asOf: "2026-08-01", bufferCents: 250000,
    forecast: fc({ shortfall: { date: "2026-09-10", balanceCents: -1500 } }),
    tracking: dueRewards([due, fresh], "2026-08-01") });
  expect(alerts.filter((a) => a.id.startsWith("tracking:")).map((a) => a.severity)).toEqual(["info"]);
...
test("no tracking passed at all -> the alert center behaves exactly as before", () => {
  expect(computeAlerts({ accounts: [acc("A", 100)], asOf: ASOF, bufferCents: 0, forecast: fc({}) })).toEqual([]);
  expect(computeAlerts({ accounts: [acc("A", 100)], asOf: ASOF, bufferCents: 0, forecast: fc({}), tracking: [] })).toEqual([]);
});
