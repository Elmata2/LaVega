...
test("rebuildVatFlows preserves non-vat flows and other entities' vat flows", () => {
  const nonVat = makeScheduledFlow({
    entity: "BV1",
    label: "invoice",
    sign: 1,
    amountCents: 7000,
  ...
  const otherEntityVat = makeScheduledFlow({
    entity: "BV2",
    label: "BTW Q2",
    sign: -1,
    amountCents: 30000,
  ...
  const shownOldVat = makeScheduledFlow({
    entity: "BV1",
    label: "BTW Q1",
    sign: -1,
    amountCents: 10000,
...
test("rebuildVatFlows dedups on recompute via content-hashed ids", () => {
  const existing = makeScheduledFlow({
    entity: "BV1",
    label: "BTW Q2",
    sign: -1,
    amountCents: 20000,
...
test("rebuildTaxFlows replaces both tax sources for the shown entities in one pass", () => {
  const oldVat = makeScheduledFlow({
    entity: "BV1",
    label: "USt Q1",
    sign: -1,
    amountCents: 100,
  ...
  const oldPrepay = makeScheduledFlow({
    entity: "BV1",
    label: "Vorauszahlung 1/4 2026",
    sign: -1,
    amountCents: 200,
    dueDate: "2026-03-10",
    source: "prepayment",
  ...
  const invoice = makeScheduledFlow({
    entity: "BV1",
    label: "factuur",
    sign: 1,
    amountCents: 300,
