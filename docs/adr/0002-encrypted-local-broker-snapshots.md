...
- Local agent-run summaries and model results also live in the encrypted vault. They can contain portfolio facts and are available only after vault unlock. The old plaintext agent-run file is no longer written. On startup, an unlocked vault migrates and then deletes that legacy file. If the vault stays locked or is empty, startup deletes the legacy file and discards its latest-run result so sensitive plaintext cannot remain on disk. The vault keeps one latest-started run, not a history.
