...
v1 ships three charts, no more: **portfolio value vs. benchmark** (line, EUR-converted, default S&P 500 overlay, 1M/6M/1Y/YTD/All range switcher), **allocation donut** (by instrument and by entity, current snapshot only), and **per-position price line** (single instrument, trade + dividend markers). No candlestick/OHLC anywhere — there is no intraday data in the pipeline, only daily EOD ([What the investing dashboard must plot](https://github.com/Elmata2/LaVega/issues/16)). This inventory is what the charting-library decision below is sized against; the portfolio-vs-benchmark chart's zoom/pan interaction is decided separately below ([Interaction model for the portfolio chart](https://github.com/Elmata2/LaVega/issues/77)).

Allocation does not group positions by broker yet, because `Position` has no broker field. The second donut mode is entity grouping (`position.entity`) until a real broker attribute is added to positions.

**Deposit-neutral return** ([Define deposit-neutral portfolio return](https://github.com/Elmata2/LaVega/issues/72)): the portfolio-vs-benchmark chart's second line is a true, chain-linked **TWR** (time-weighted return), not the rescaled-price approximation `normalizeBenchmarkSeries` computes today. Sub-periods are bounded by each `CashFlow` date; flows apply end-of-day, so a day's return is priced before the flow lands and the next sub-period starts from the post-flow value. `Dividend` is not a boundary — it's already inside `cashValue` (folded per issue #71), so it flows into the return like any other gain; treating it as an external flow would double-count it. Dates before `cashOnDate` is reachable (`cashUnknown`) return `null`, same gap convention as elsewhere. New type: `ReturnPoint = { date: string; cumulativeReturn: number | null }`, for both the portfolio and the benchmark (each normalized to its own start, not rescaled to the other's value).

Alongside TWR, the chart's crosshair also reads **MWR** as **XIRR** — a root-find over dated flows, pure/local, no new dependency — computed **since the selected range's start**, not since inception, matching what `filterPortfolioValueRange` already scopes the TWR line to. Inputs are `CashFlow` `deposit`/`withdrawal` only (`interest`/`fee`/`other` are internal, `Dividend`/`Trade` excluded — same reasoning as the TWR ruling above) plus a synthetic flow at the window's start: the range-start `positionsValue + cashValue` as an implicit outflow, so XIRR is solvable even in windows with no real deposits, and tracks intra-window price appreciation. The benchmark gets the same treatment, replaying the same flow amounts (including the synthetic start) as if invested in the benchmark at its price on each date — "what if this money had bought the benchmark instead." New type: `MwrPoint = { date: string; xirr: number | null }`, portfolio and benchmark. Same `cashUnknown`/solver-non-convergence `null` gap convention as TWR. Solver method, tolerance, and perf strategy are implementation, not spec.
...
**Portfolio chart interaction model** ([Interaction model for the portfolio chart](https://github.com/Elmata2/LaVega/issues/77)): drag directly on the chart plot to zoom into a date range, plus scroll-wheel zoom (hover the chart and scroll to zoom in/out smoothly, in ~5%-of-window steps per tick; scrolling anywhere else leaves the page alone) and a typed-date-range fallback (`from`/`to` date inputs) — all three write the same zoom state, so the zoom pill and Escape clear whichever one set it. Rejected: a separate `<Brush>` strip below the chart, because repeated drags on it compound the zoom further inward with no way to reposition within the current window short of clearing it — the single drag-on-plot control doesn't have that trap. Crosshair, tooltip (flips near the right edge), keyboard nav (Tab into the chart, `←`/`→`/Home/End move the crosshair, Escape clears zoom), and click-to-drill on a trade marker are unchanged by which zoom control is used. Prototype: [`prototype-interaction-77`](https://github.com/Elmata2/LaVega/tree/prototype-interaction-77/apps/investing-chart-prototype).
