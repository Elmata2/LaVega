...
**Local/self-hosted tier default: Yahoo Finance (unofficial), shown behind a one-time disclosure at first use.** This reverses the research recommendation of an opt-in, consent-gated integration — the reasoning for defaulting is recorded in [ADR 0001](../adr/0001-yahoo-finance-default-price-source.md) rather than left implicit. Yahoo covers what LaVega needs — Euronext Amsterdam, XETRA, Paris, and London all reachable by ticker suffix, plus indices and FX pairs — for free, with no API key. Its endpoints are undocumented and reverse-engineered, rate-limited and IP-blocked informally, and can break without notice; the user is told once, in plain language, the same one-time-nag pattern `CONNECTORS.md` already uses for broker risk disclosure ([Market-data provider](https://github.com/Elmata2/LaVega/issues/34)).

**Optional local alternative: EODHD** "EOD Historical Data — All World" (~€19.99/mo, self-funded, bring-your-own key). Bundles the ISIN/CUSIP/FIGI ID Mapping API for the broker-instrument bridge. Whether it stays worth offering now that Yahoo covers the free-default case is still open ([Does EODHD remain an optional local provider?](https://github.com/Elmata2/LaVega/issues/37)).
