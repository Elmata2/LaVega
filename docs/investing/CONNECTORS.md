...
Per-broker failures go in `problems` so one broken connection doesn't block the rest, exactly as `BankAccessAdapter` does it. `BrokerResult.sections` contains one `{status, rows}` value per replaceable data section. `complete` includes successful empty results, `partial` includes usable rows with known gaps, and `unavailable` preserves last-good cache rows. An adapter that can't reach its broker marks every section `unavailable`; it does not throw.

**Cash-history coverage** ([FIN-08, #113](https://github.com/Elmata2/LaVega/issues/113)). `BrokerResult.cashHistory` is a `CashHistoryCoverage`: what the returned cash sections prove about past cash. `complete` carries `from` and `to`, the first and last dates between which every cash movement is in the returned records, and `tradeCash`, the one stream that records what a trade moved: `trade-settlement` (the trade's own `settlement`) or `cash-flows` (ledger rows among the cash flows). The core cash walk counts trade cash from that stream only, so a broker that books both is never counted twice. `unknown` carries a reason, and so does an absent value. Outside a complete window the core does not walk cash flows. A broker balance date is known, and the last known balance (the latest broker balance, or the balance walked to `to` when that is later) carries forward to later dates, including a balance dated on a weekend or on an earlier UTC day than today. It stops at the first cash flow, dividend or trade of the same entity and broker dated after it: from that date on, cash is in `cashUnknown` until a newer broker balance. A trade stops every currency wallet of that broker, because its settlement currency is not proven. Every date before or between broker balances is reported in `cashUnknown`. The first retained cash event is not evidence that nothing moved before it. The encrypted snapshot stores coverage beside the cash it describes: a sync that read any cash section replaces it, a sync that read none keeps it, and snapshots written before coverage existed restore as unknown.

| Broker      | Coverage                                                                                                                                                                                                                                                                                                                                             | Evidence                                                                                                                                                                                                                                                                                                  |
| ----------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| IBKR        | `complete` from the `FlexStatement` `fromDate` to its `toDate`, `tradeCash: "cash-flows"`, when Cash Report and Statement of Funds are both complete. With several account statements, every statement must carry both sections and cover the same period, because Cash Report rows merge across accounts by currency and date. Otherwise `unknown`. | Checked-in synthetic Flex fixtures (`flexParser.test.ts`, `cashHistory.test.ts`) book trade cash and commissions as Statement of Funds rows beside the `Trade` rows, so trades do not settle a second time.                                                                                               |
| Trading 212 | Always `unknown`.                                                                                                                                                                                                                                                                                                                                    | No sanitized live payload is checked in. Fixtures show `walletImpact.netValue` on fills (account currency, net of taxes) and deposit, withdrawal, fee and interest rows on `/history/transactions`. They do not show whether transactions also book execution cash, or how far either stream is retained. |

To make Trading 212 `complete`, check in a sanitized `/history/orders` and `/history/transactions` pair for the same period with at least one buy, one sell, a foreign-currency fill and a fee. Record which stream books execution cash, in which currency, and the earliest date both streams return. Then set `from` and `tradeCash` from that evidence.
...
A first history is tens of pages. On Vercel the function `maxDuration` is 300s and the adapter stops ~60s before that (`INVESTING_SYNC_BUDGET_MS`, default 240s on the hosted tier, 45s when `VERCEL` is set without a budget). The route creates this absolute deadline once and passes it unchanged through broker orchestration and price synchronization. Local calls without a deadline remain unlimited. Trading 212 does **not** sleep a 60s provider window that would overrun the host; it returns the pages already read, a `resume` cursor (`ordersNextPagePath`), and `retryAfter`. The next invocation continues from that cursor instead of restarting page one. Positions and account summary run first so a cut-off run still persists live holdings and cash. Unavailable holdings or cash sections do not replace last-good rows with empty arrays. Partial history merges rows by stable broker identity until complete.

IBKR uses the same request deadline and five-second persistence margin. It checks before requesting a Flex statement, before the initial generation wait, before each poll request, and before each poll sleep. When time is short, the adapter returns an unavailable problem result. The scheduler can persist other completed broker results, and the next sync retries IBKR.

**Cash history:** `GET /api/v0/equity/account/summary` anchors available cash. Transaction and dividend endpoints follow every returned `nextPagePath` without date assumptions, deduplicate stable `reference` values, and persist normalized records in encrypted broker snapshots. They start only after order history is complete, so a resumed run does not spend the 6/min budget on a second stream while trades are still incomplete. Malformed rows and incomplete pagination become visible problems. A partial sync merges the pages it did read into the last snapshot and does not replace a complete history with a truncated one. `TRANSFER` direction, provider sign behavior, and account-specific retention still require one sanitized live-response check. Until then, ambiguous transfers are not invented, and non-zero `inPies` or `reservedForOrders` prevents treating `availableToTrade` as total cash. Complete pagination does not prove complete cash history. The adapter reports `cashHistory` as `unknown`, so historical Trading 212 cash stays unknown instead of being walked through fills and transactions that may overlap. The latest account summary balance still values every date from its sync day onwards.

**Price synchronization: budgeted slices, driven by the caller or cron.** Yahoo price history runs inside the request that asks for it, never after the response — work detached with `void` after a serverless function replies is not guaranteed to run at all, and for months it did not, leaving 48 symbols without a current bar while every call still returned 200. One call syncs as many symbols as `INVESTING_PRICE_SYNC_BUDGET_MS` (falling back to `INVESTING_SYNC_BUDGET_MS`, then 45s under `VERCEL`, then unlimited locally) allows, counted from where the request started, so a broker sync that already spent the host budget leaves a deadline that has passed and the price run pauses instead of overrunning `maxDuration`. The hosted budget is 60s rather than the broker's 240s, because a proxy closes a request at ~100s and a round that never answers cannot be read. A paused run answers `202` with `status: "paused"` and the symbols it did not reach; the client posts `/api/prices/sync` again until the status is `completed` or `problem`, for at most 40 rounds; a run still paused after that ends as an explicit `incomplete` outcome that the dashboard shows. The status poll only reads the row; it never resumes a paused run, so the web app polls a paused row only while it is posting the next round itself. Each round is shorter than the last, because a symbol whose bars are stored costs no provider request. Progress lives in `investing.sync_state` under the key `prices`, so the status poll is answered from the row rather than from one instance's memory, and a run interrupted by the host has something to resume from. The row also carries a `leaseId`; the database claim is atomic, and later progress writes only succeed for the current lease. A fresh `running` or `waiting` row blocks duplicate provider traffic; a stale one can be claimed by the next invocation.

**Hosted cron driver:** the Vercel build output registers `GET /api/cron/investing-sync` at `0 4 * * *` UTC. The route requires `Authorization: Bearer $CRON_SECRET`. With authentication enabled it syncs only tenants listed in `INVESTING_CRON_TENANT_IDS`; without authentication it falls back to the single local tenant. Each cron pass runs broker sync, then starts one fresh price slice, so normal daily top-ups happen without opening the dashboard. Manual dashboard-open sync still exists and resumes any paused price row.
