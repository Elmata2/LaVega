...
**Rejected keys stop the sync at the first endpoint.** HTTP 401 (bad key) and HTTP 403 (missing scope) are properties of the stored key pair, not of the endpoint that returned them: retrying cannot fix either, and every other endpoint answers the same way. The adapter therefore abandons the remaining reads, leaves all sections `unavailable` so last-good rows survive, and reports one problem naming the reconnect the user has to do. It no longer emits four copies of a raw `HTTP 401` line, one per endpoint.
