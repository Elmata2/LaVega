...
### Task 1: `apps/server` Hono scaffold + config loading _(not gated)_
...
### Task 2: EB JWT + API client _(not gated — test with a generated test key)_
...
### Task 3: EB routes — aspsps / auth / callback / sync / forget _(gated on G1 to run live; structure buildable now)_
...
### Task 4: Pure mapping `mapEbTransaction` / `mapEbAccount` _(not gated — fully testable now)_
...
### Task 5: `EnableBankingAdapter` (BankAccessAdapter impl) _(gated on G1 to run; wiring testable now)_
...
### Task 6: Web UI — "Live bankkoppeling" _(gated on G1 for real SCA; UI + wiring buildable now)_
