...
export async function deriveKey(
  ...
  salt: Uint8Array,
  iterations: number,
): Promise<CryptoKey>; // AES-GCM 256, non-extractable
export async function encryptJSON(
  key: CryptoKey,
  salt: Uint8Array,
  iterations: number,
  data: unknown,
): Promise<CipherBlob>; // fresh 12-byte IV
...
export interface VaultStorage extends StorageAdapter {
  ...
  ): Promise<void>; // create vault (optionally from migrated plaintext), unlock
...
- On load, `status()`:
  - **empty + legacy plaintext present** (existing `lavega` v2 DB has data) → **migration flow**: show a "Beveilig je data" screen: warn _"wachtwoord kwijt = data kwijt"_, require passphrase (twice) + a checkbox "ik heb dit begrepen", then `setup(pass, seed=<read from plaintext>)`, verify unlock, delete the plaintext `lavega` DB, and **prompt/force a back-up** (Task 5). Never delete plaintext before the vault verifies.
