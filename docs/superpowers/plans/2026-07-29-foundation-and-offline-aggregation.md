...
export default defineConfig({ test: { environment: "node", include: ["packages/**/*.test.ts", "apps/**/*.test.ts"] } });
```
...
test("core loads", () => { expect(VERSION).toBe("0.0.0"); });
```
...
const base = { accountKey: "NL01", date: "2026-01-02", amount: -10, currency: "EUR",
  counterparty: "Shop", description: "x", category: "", manual: false };
...
const mk = (o:Partial<any>)=>({ accountKey:"A", date:"2026-01-02", amount:-10, currency:"EUR",
  counterparty:"S", description:"d", category:"", manual:false, ...o });
...
test("consolidate sums in/out per entity", () => {
  ...
  const accounts = [{ key:"A", iban:"A", name:"", bank:"", entity:"BV1", currency:"EUR", balance:100 }];
...
export function consolidate(accounts: Account[], txs: Tx[]) {
  ...
  for (const t of txs) {
    ...
    if (t.amount >= 0) byEntity[e].in += t.amount; else byEntity[e].out += t.amount;
  }
