...
test("getSession returns null when auth is not configured (503)", async () => {
  vi.stubGlobal("fetch", vi.fn(() => Promise.resolve(new Response(JSON.stringify({ problems: ["Authentication is not configured"] }), { status: 503 }))));
...
test("getSession returns anonymous when configured but no cookie", async () => {
  ...
  expect(fetch).toHaveBeenCalledWith("/api/auth/get-session", { headers: { accept: "application/json" } });
});
...
test("signUp posts name, email and password to sign-up/email", async () => {
  ...
  const result = await signUp({ name: "Jort", email: "jort@example.com", password: "correct horse battery staple" });
...
test("signIn posts email and password to sign-in/email", async () => {
  const fetchMock = vi.fn(() => Promise.resolve(new Response(JSON.stringify({ user: { id: "u1" } }), { status: 200 })));
  vi.stubGlobal("fetch", fetchMock);
  const result = await signIn({ email: "jort@example.com", password: "correct horse battery staple" });
  expect(result).toEqual({ ok: true });
...
test("signIn surfaces a fallback message when the server sends none", async () => {
  vi.stubGlobal("fetch", vi.fn(() => Promise.resolve(new Response(JSON.stringify({}), { status: 401 }))));
