...
type PositionPriceChartProps = {
  ...
  onMarkerActivate?: (date: string) => void;
};
const markerColors = {
  buy: "hsl(var(--pos))",
  sell: "hsl(var(--neg))",
  dividend: "hsl(var(--chart-amber))",
} as const;
...
function chartPointsWithGaps(
  points: PositionPricePoint[],
): Array<PositionPricePoint | { date: string; close: null }> {
  ...
  for (const [index, point] of points.entries()) {
    ...
    if (previous) {
      ...
      while (cursor < end) {
        ...
        if (day !== 0 && day !== 6)
          result.push({ date: cursor.toISOString().slice(0, 10), close: null });
        cursor.setUTCDate(cursor.getUTCDate() + 1);
      }
    }
    result.push(point);
...
export function PositionPriceChart({
  ...
  const presetPoints = useCallback(
    (range: Parameters<typeof positionPointsForRange>[1]) => positionPointsForRange(all, range),
    [all],
  );
  ...
  const [prevWindow, setPrevWindow] = useState(chart.window);
  if (chart.window !== prevWindow) {
    setPrevWindow(chart.window);
  ...
  const markers = visible.flatMap((point, pointIndex) =>
    point.markers.map((marker, markerIndex) => ({ marker, point, pointIndex, markerIndex })),
  );
        ...
        >
          {chartRanges.map((range) => (
            <button
              key={range.value}
              type="button"
              aria-pressed={chart.window.kind === "preset" && chart.window.range === range.value}
              onClick={() => {
                chart.applyPreset(range.value);
                setActiveEventDate(null);
              }}
              className="pressable rounded-pill px-2.5 py-1.5 text-xs font-semibold text-muted-foreground hover:text-foreground"
            >
              {range.label}
            </button>
          ))}
          ...
          <>
            ...
            >
              <ChartContainer className="h-[300px]" aria-hidden="true">
                <LineChart data={plotPoints} margin={{ top: 12, right: 12, left: 0, bottom: 0 }}>
                  ...
                  {markers.map(({ marker, point, markerIndex }) => (
                    <ReferenceDot
                      key={`${marker.eventDate}-${marker.kind}-${markerIndex}`}
                      x={point.date}
                      y={point.close}
                      r={markerIndex === 0 ? 6 : 4}
                      fill={markerColors[marker.kind]}
            ...
            {active && (
              <div className="mt-3">
                <MarkerDetails
                  close={active.close}
                  date={activeEventDate ?? active.date}
                  markers={
                    activeEventDate
                      ? active.markers.filter((marker) => marker.eventDate === activeEventDate)
                      : active.markers.filter((marker) => marker.eventDate === active.date)
                  }
                  currency={currency}
                />
              </div>
              ...
              {visible.map((point) => (
                <li key={point.date}>
                  ...
                  {point.markers.length
                    ? `, ${point.markers.map((marker) => marker.label).join(", ")}`
                    : ""}
                </li>
              ))}
