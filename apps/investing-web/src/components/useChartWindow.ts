...
export function useChartWindow<T extends Dated>(options: {
  ...
  const points = useMemo(
    () => pointsInWindow(allPoints, window, presetPoints),
    [allPoints, presetPoints, window],
  );
  ...
  const clearZoom = useCallback(() => {
    setWindow((current) =>
      current.kind === "custom" ? { kind: "preset", range: current.baseRange } : current,
    );
    resetFocus();
  }, [resetFocus]);
  const indexForClientX = useCallback(
    (clientX: number, count = points.length) => {
  ...
  useEffect(() => {
    ...
    const onWheel = (event: WheelEvent) => {
      ...
      const nextCount = Math.max(
        minimumWheelPoints,
        Math.min(allPoints.length, points.length + (event.deltaY < 0 ? -step : step)),
      );
      if (nextCount >= allPoints.length) {
        clearZoom();
        return;
      }
      const centerDate =
        points[Math.round(ratio * (points.length - 1))]?.date ?? points.at(-1)!.date;
      ...
      setWindow({
        kind: "custom",
        from: allPoints[start]!.date,
        to: allPoints[start + nextCount - 1]!.date,
        baseRange,
      });
      setFocusIndex(null);
    };
  ...
  const onKeyDown = useCallback(
    (event: React.KeyboardEvent) => {
      ...
      if (event.key === "ArrowRight") {
        event.preventDefault();
        setFocusIndex((current) => Math.min(last, (current ?? last - 1) + 1));
      }
      if (event.key === "ArrowLeft") {
        event.preventDefault();
        setFocusIndex((current) => Math.max(0, (current ?? last + 1) - 1));
      }
      if (event.key === "Home") {
        event.preventDefault();
        setFocusIndex(0);
