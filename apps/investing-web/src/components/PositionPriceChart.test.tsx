        ...
        points={[
          {
            ...
            close: 100,
            currency: "USD",
            markers: [{ kind: "buy", eventDate: "2026-01-05", label: "Koop 2" }],
          },
        ]}
      />,
    );
...
test("moves exact-value crosshair with keyboard and clears zoom with Escape", async () => {
  const points = ["2026-01-05", "2026-02-10", "2026-03-20"].map((date, index) => ({
    symbol: "AAPL",
    date,
    close: 100 + index,
    currency: "USD",
  ...
  const root = createRoot(container);
  await act(async () => {
    root.render(<PositionPriceChart symbol="AAPL" currency="USD" points={points} />);
  });
