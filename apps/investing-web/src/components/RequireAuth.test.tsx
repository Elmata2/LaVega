...
test("renders protected content when auth is not configured", async () => {
  expect(await render(503, { problems: ["Authentication is not configured"] })).toContain(
    "Beveiligde inhoud",
  );
});
...
test("renders protected content when a session exists", async () => {
  expect(
    await render(200, { session: { id: "s1" }, user: { id: "u1", email: "jort@example.com" } }),
  ).toContain("Beveiligde inhoud");
});
