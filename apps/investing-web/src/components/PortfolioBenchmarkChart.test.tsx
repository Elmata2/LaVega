...
test("uses one custom window for typed dates and clears it with Escape", async () => {
  ...
  await act(async () => { root.render(<PortfolioBenchmarkChart data={{ "1M": longPoints, All: longPoints }} benchmarks={benchmarks} />); await Promise.resolve(); });
  ...
  await act(async () => { container.querySelector<HTMLFormElement>('form[aria-label="Datumbereik kiezen"]')!.dispatchEvent(new Event("submit", { bubbles: true, cancelable: true })); });
  expect(container.querySelector('button[aria-label="Zoom wissen"]')?.textContent).toContain("3 jan");
...
test("supports keyboard crosshair and announces exact unknown values", async () => {
  ...
  for (let index = 0; index < 4; index += 1) await act(async () => { chart.dispatchEvent(new KeyboardEvent("keydown", { key: "ArrowRight", bubbles: true })); });
  expect(container.querySelector('[aria-live="polite"]')?.textContent).toContain("MISSING");
  expect(container.querySelector('ul[aria-label="Exacte grafiekwaarden"]')?.textContent).toContain("AEX TWR");
  await act(async () => root.unmount());
});
...
test("wheel and pointer drag write custom zoom without brush", async () => {
  ...
  expect(container.querySelector('button[aria-label="Zoom wissen"]')?.textContent).not.toBe(wheelWindow);
  await act(async () => root.unmount());
});

test("window helper preserves original requested start", () => {
  expect(pointsForWindow({ All: longPoints }, { kind: "custom", from: "2026-01-03", to: "2026-01-05", baseRange: "1M" }).map(({ date }) => date)).toEqual([
