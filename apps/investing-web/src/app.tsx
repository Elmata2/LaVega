...
export function YahooDisclosure() {
  ...
  useEffect(() => { fetch("/api/market-data/consent").then(async (response) => await response.json() as Consent).then(setConsent).catch(() => setProblem("Kan toestemmingsstatus niet laden.")); }, []);
...
function Overview() { return <div className="space-y-5"><YahooDisclosure /><div className="grid gap-5 lg:grid-cols-[1.35fr_.65fr]"><Card><CardHeader><p className="text-sm font-medium text-muted-foreground">Portefeuillewaarde</p><CardTitle>Nog geen gegevens</CardTitle></CardHeader><CardContent><EmptyState title="Jouw portefeuille wacht" description="Koppel een broker om hier portefeuillewaarde, rendement en benchmarkgegevens te zien." /></CardContent></Card><Card><CardHeader><p className="text-sm font-medium text-muted-foreground">Verdeling</p><CardTitle>Begin met jouw posities</CardTitle></CardHeader><CardContent><EmptyState title="Geen posities" description="Jouw verdeling verschijnt na de eerste brokersynchronisatie." /></CardContent></Card></div></div>; }
