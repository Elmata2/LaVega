...
function PositionDetailSummary({ position }: { position: InvestingPositionDetail }) {
  ...
  const available = position.returnStatus === "available" || position.returnStatus === "broker-average";
  return <section aria-labelledby="position-title" className="rounded-card border border-border bg-card p-5 shadow-soft sm:p-6">
    ...
    {position.returnStatus === "broker-average" && <p role="status" className="mt-4 rounded-[14px] border border-warning/30 bg-warning/10 px-4 py-3 text-sm">Kostprijs komt van de gemiddelde aankoopprijs van je broker, omdat niet elke aankoop in de orderhistorie staat. Gerealiseerde winst en jaarrendement kunnen daardoor niet worden berekend.</p>}
