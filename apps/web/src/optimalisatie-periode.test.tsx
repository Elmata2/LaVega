...
function schakel(el: HTMLElement, waarde: string) {
  const select = el.querySelector<HTMLSelectElement>(
    'select[aria-label="Eenheid van de abonnementsbedragen"]',
  );
...
test("naar jaar wordt vermenigvuldigd en naar maand gedeeld — nooit andersom", () => {
  ...
  expect(amountInPeriod(1_799, 30, "maand")).toEqual({
    kind: "bedrag",
    cents: 1_799,
    derived: false,
    sum: null,
  });
  expect(amountInPeriod(1_799, 30, "jaar")).toEqual({
    kind: "bedrag",
    cents: 21_588,
    derived: true,
    sum: `12 \u00d7 ${eu("17,99")}`,
  ...
  expect(amountInPeriod(12_000, 365, "jaar")).toEqual({
    kind: "bedrag",
    cents: 12_000,
    derived: false,
    sum: null,
  });
  expect(amountInPeriod(12_000, 365, "maand")).toEqual({
    kind: "bedrag",
    cents: 1_000,
    derived: true,
    sum: `${eu("120,00")} \u00f7 12`,
  ...
  for (const days of [30, 61, 91, 182, 365]) {
    expect(amountInPeriod(4_500, days, "eigen")).toEqual({
      kind: "bedrag",
      cents: 4_500,
      derived: false,
      sum: null,
...
test("een ritme dat we niet kennen wordt niet geraden en telt niet als nul mee", () => {
  ...
  expect(subAmountIn(raar, "eigen")).toEqual({
    kind: "bedrag",
    cents: 1_000,
    derived: false,
    sum: null,
...
test('een totaal heeft een noemer nodig, dus "zoals afgeschreven" telt per jaar op', () => {
  ...
  expect(subsTotalIn([kwartaal, maand], "eigen")).toEqual({
    cents: 39_588,
    unit: "jaar",
    onbekend: 0,
  });
  expect(subsTotalIn([kwartaal, maand], "jaar")).toEqual({
    cents: 39_588,
    unit: "jaar",
    onbekend: 0,
  });
  expect(subsTotalIn([kwartaal, maand], "maand")).toEqual({
    cents: 1_500 + 1_799,
    unit: "maand",
    onbekend: 0,
  });
...
test("wat er is omgerekend staat geteld in het label, en de som per rij erin", () => {
  ...
  expect(perMaand?.textContent).toContain(
    `${eu("45,00")} per kwartaal \u2192 ${eu("45,00")} \u00f7 3 = ${eu("15,00")} per maand`,
  );
...
test("een prijsstijging wordt niet meer blind × 12 gerekend", () => {
  const el = mount();
  const zin = () =>
    [...el.querySelectorAll("p.reason")].find((p) => p.textContent?.includes("Netflix"))
      ?.textContent ?? "";
...
test("een stijging op een KWARTAALafschrijving is vier keer per jaar, niet twaalf", () => {
  ...
  const zin = () =>
    [...el.querySelectorAll("p.reason")].find((p) => p.textContent?.includes("Simeo"))
      ?.textContent ?? "";
  schakel(el, "jaar");
