...
test("TopUitgavenBlock ranks the latest month's categories with share and delta", () => {
  const html = renderToStaticMarkup(
    <TopUitgavenBlock txs={txs} rules={rules} own={own} onSelectCategory={() => {}} onAsk={() => {}} />,
  );
  ...
  expect(html).toContain("vs. gem.");
});

test("TopUitgavenBlock renders an empty state with no spending", () => {
  const html = renderToStaticMarkup(
    <TopUitgavenBlock txs={[]} rules={rules} own={own} onSelectCategory={() => {}} onAsk={() => {}} />,
  );
