...
import { bannerState, confidenceLabel, coverageNotes, hasBand, isThinData, splitDrivers } from "./forecast-view";
...
test("coverageNotes: an account with a balance but no transactions is said out loud", () => {
  const accounts: Account[] = [
    { key: "A1", iban: "A1", name: "Betaal", bank: "ING", entity: "BV1", currency: "EUR", balance: 1000 },
    { key: "B1", iban: "B1", name: "Zakelijk", bank: "Knab", entity: "BV1", currency: "EUR", balance: 9000 },
  ];
...
test("coverageNotes: three weeks of one card among months of another is named, with the number", () => {
  ...
  expect(coverageNotes(f).find((n) => n.id === "short-account")!.text).toContain("21 dagen historie");
...
test("coverageNotes: a stale import says how old it is instead of quietly forecasting from it", () => {
  const txs: Tx[] = [
    tx("1", "2026-01-05", 3000, "Werkgever"), tx("2", "2026-02-05", 3000, "Werkgever"), tx("3", "2026-03-05", 3000, "Werkgever"),
  ];
  const accounts: Account[] = [{ key: "A1", iban: "A1", name: "x", bank: "", entity: "BV1", currency: "EUR", balance: 5000 }];
  const f = forecastCashflow(txs, accounts, { asOf: "2026-04-20" }).byEntity["BV1"];
  expect(coverageNotes(f).find((n) => n.id === "stale-import")!.text).toContain("2026-03-05, 46 dagen geleden");
});

test("coverageNotes: a forecast without a basis (hand-written fixture) says nothing at all", () => {
  expect(coverageNotes({ scope: "x", asOf: "2026-08-01", horizonDays: 91, openingCents: 0, points: [], shortfall: null, streams: [], drivers: [] })).toEqual([]);
});

test("hasBand: false when the engine could not measure one, true once it could", () => {
  const accounts: Account[] = [{ key: "A1", iban: "A1", name: "x", bank: "", entity: "BV1", currency: "EUR", balance: 5000 }];
  const thin = forecastCashflow(
    [tx("1", "2026-06-10", -10000, "Equipment"), tx("2", "2026-06-20", -50, "Coffee"), tx("3", "2026-06-30", -50, "Coffee")],
    accounts, { asOf: "2026-07-01" },
  ).byEntity["BV1"];
  expect(hasBand(thin)).toBe(false);

  const measured = forecastCashflow(
    [tx("1", "2026-04-25", 2500, "Werkgever"), tx("2", "2026-05-25", 2600, "Werkgever"), tx("3", "2026-06-25", 2400, "Werkgever")],
    accounts, { asOf: "2026-07-01" },
  ).byEntity["BV1"];
