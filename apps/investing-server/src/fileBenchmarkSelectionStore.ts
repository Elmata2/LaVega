...
export function createFileBenchmarkSelectionStore(filePath: string): BenchmarkSelectionStore {
    ...
    validate: (contents) => {
      ...
      return parsed.map((row) => {
        if (
          !row ||
          typeof row !== "object" ||
          typeof (row as BenchmarkSelection).tenantId !== "string" ||
          !Array.isArray((row as BenchmarkSelection).symbols)
        )
          throw new Error("Invalid benchmark selection row");
  ...
  return {
    async get(tenantId) {
      return (
        (await store.read()).find((row) => row.tenantId === tenantId) ?? { tenantId, symbols: [] }
      );
    },
    async set(selection) {
      const normalized = {
