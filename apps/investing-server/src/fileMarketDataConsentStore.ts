...
export function createFileMarketDataConsentStore(
  filePath = runtimeMarketDataConsentFile(),
  ...
  const store = createJsonFileStore<MarketDataConsentDecision[]>(filePath, {
    ...
    validate: (contents) => {
      ...
      return parsed.map((row) => {
        ...
        if (
          typeof decision.tenantId !== "string" ||
          ...
          throw new Error("Invalid market-data consent row");
  ...
  return {
    ...
    async set(decision) {
      await store.update((rows) => [
        ...rows.filter((row) => row.tenantId !== decision.tenantId),
        decision,
      ]);
    },
  };
