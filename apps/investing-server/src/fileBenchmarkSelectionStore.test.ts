...
test("persists ordered benchmark selection across store instances", async () => {
  ...
  await createFileBenchmarkSelectionStore(file).set({ tenantId: "local", symbols: ["^AEX", "^GDAXI"] });
  await expect(createFileBenchmarkSelectionStore(file).get("local")).resolves.toEqual({ tenantId: "local", symbols: ["^AEX", "^GDAXI"] });
});
